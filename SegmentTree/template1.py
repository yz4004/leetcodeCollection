...
class NumArray:
    ...
    def build(self, p, l, r):
        ...
        self.t[p] = max(self.t[2*p], self.t[2*p+1])


    """ index 上加 val """
    def add(self, p, l, r, index, val):
        if l == r:
            self.t[p] = max(self.t[p], val)
            return
        mid = (l+r)//2
        self.t[p] = max(self.t[p], val)
        if index <= mid:
    ...
    def query(self, p, l, r, left, right):
        if l > right or r < left: return float("-inf")
        if left <= l and r <= right:
            return self.t[p]
        mid = (l+r)//2
        return max(self.query(2*p, l, mid, left, right), self.query(2*p+1, mid+1, r, left, right))
